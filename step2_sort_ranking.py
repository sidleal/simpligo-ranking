...
class Quick(object):
    def particao(self, a, ini, fim):
        ...
        for i in range(ini, fim):
            ...
            input_a = np.array(pivo['feats'])
            input_b = np.array(a[i]['feats'])
            prediction = pipeline_pss.predict([np.array([input_a]), np.array([input_b])])

            #input = np.append(pivo['feats'],a[i]['feats'])
            #prediction = pipeline_pss.predict(np.array([input]))
            #print(pivo)
            #print(a[i])
...
# load dataset
df = pandas.read_csv("data/pss2_features_pairs_align2.tsv", delimiter='\t', header=0)
...
pipeline_pss = load_model("models/model_pss2_v2_10.h5")

df_from = df[feats_from]
df_to = df[feats_to]
df_from_norm = (df_from - df_from.min()) / (df_from.max() - df_from.min())
df_to_norm = (df_to - df_to.min()) / (df_to.max() - df_to.min())

new_list = []
for i, row in df.iterrows():
    line_from = {'level': '', 'text': df.iloc[i, 4], 'feats': df_from_norm.iloc[i, :], 'feats_raw': df_from.iloc[i, :]}
    line_to = {'level': '', 'text': df.iloc[i, 194], 'feats': df_to_norm.iloc[i, :], 'feats_raw': df_to.iloc[i, :]}
...
print(new_list[0]['text'])
print(new_list[1]['text'])

a = np.array(new_list[0]['feats'])
b = np.array(new_list[1]['feats'])
print(a.shape, b.shape)
prediction = pipeline_pss.predict([np.array([a]), np.array([b])])
print(prediction)

a = np.array(new_list[1]['feats'])
b = np.array(new_list[0]['feats'])
print(a.shape, b.shape)
prediction = pipeline_pss.predict([np.array([a]), np.array([b])])
print(prediction)
...
dest = open("data/pss2_ranking_global_v3_raw.tsv", "w", encoding="utf-8")

dest.write('idx\tlevel\ttext')
