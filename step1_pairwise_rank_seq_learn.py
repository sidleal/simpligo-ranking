...
def pss_model():
    input_layer_pss_1 = Input(shape=(156,), name='pss_input_1')
    input_layer_pss_2 = Input(shape=(156,), name='pss_input_2')

    layer_1_pss_1 = Dense(64, kernel_initializer='normal', activation='relu', name='layer_1_pss_1')(input_layer_pss_1)
    layer_1_pss_2 = Dense(64, kernel_initializer='normal', activation='relu', name='layer_1_pss_2')(input_layer_pss_2)

    shared_layer_1_pss_1 = shared_layer_1(input_layer_pss_1)
    shared_layer_1_pss_1.trainable = False
    shared_layer_1_pss_2 = shared_layer_1(input_layer_pss_2)
    shared_layer_1_pss_2.trainable = False

    shared_layer_2_pss_1 = shared_layer_2(shared_layer_1_pss_1)
    shared_layer_2_pss_1.trainable = False
    shared_layer_2_pss_2 = shared_layer_2(shared_layer_1_pss_2)
    shared_layer_2_pss_2.trainable = False

    eye_predict_pss_1 = Dense(3, kernel_initializer='normal', activation="relu", name='eye_predict_pss_1')(shared_layer_2_pss_1)
    eye_predict_pss_2 = Dense(3, kernel_initializer='normal', activation="relu", name='eye_predict_pss_2')(shared_layer_2_pss_2)

    merged_pss_1 = layers.concatenate([layer_1_pss_1, eye_predict_pss_1])
    merged_pss_2 = layers.concatenate([layer_1_pss_2, eye_predict_pss_2])
    merged_pss = layers.concatenate([merged_pss_1, merged_pss_2])

    output_layer_pss = Dense(1, kernel_initializer='normal', activation="sigmoid", name='pss_output')(merged_pss)

    model = Model(inputs=[input_layer_pss_1, input_layer_pss_2], outputs=output_layer_pss)
    model.compile(loss='mean_squared_error', optimizer='adam')
    return model
...
X_pss_1 = df_pss[feats_from]
X_pss_2 = df_pss[feats_to]
Y_pss = df_pss['inverse']

#normalize
X_pss_1 = (X_pss_1-X_pss_1.min())/(X_pss_1.max()-X_pss_1.min())
X_pss_2 = (X_pss_2-X_pss_2.min())/(X_pss_2.max()-X_pss_2.min())
...
count = 0
n_split = 10
for train_index, test_index in KFold(n_splits=n_split, shuffle=True).split(X_pss_1):
    count += 1
    X_train_pss_1, X_train_pss_2, Y_train_pss = X_pss_1.iloc[train_index], X_pss_2.iloc[train_index], Y_pss.iloc[train_index]
    X_test_pss_1, X_test_pss_2, Y_test_pss = X_pss_1.iloc[test_index], X_pss_2.iloc[test_index], Y_pss.iloc[test_index]

    pipeline_pss.fit([X_train_pss_1, X_train_pss_2], Y_train_pss, epochs=30, batch_size=10, verbose=0)

    prediction = pipeline_pss.predict([X_test_pss_1, X_test_pss_2])
    ...
    pipeline_pss.save("models/model_pss2_v2_%d.h5" % count)
...
test_list = [0, 5, 32, 50, 112, 323, 456, 2]
for t in test_list:
    print(t, " --> ", Y_pss.iloc[t])
    prediction = pipeline_pss.predict([np.array([X_pss_1.iloc[t,:]]), np.array([X_pss_2.iloc[t,:]])])
    print("a-b", prediction)

    prediction = pipeline_pss.predict([np.array([X_pss_2.iloc[t,:]]), np.array([X_pss_1.iloc[t,:]])])
    print("b-a", prediction)
