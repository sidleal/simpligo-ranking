...
def baseline_model():
    ...
    output_layer_eye = Dense(1, kernel_initializer='normal', activation="relu", name='eye_output')(shared_layer)
...
estimator = KerasRegressor(build_fn=baseline_model, epochs=30, batch_size=10, verbose=1)
estimators = []
...
for z in range (0, folds):
    X_train, X_test, y_train, y_test = train_test_split(X_pss, Y_pss, test_size=0.1)
    X_train_eye, X_test_eye, y_train_eye, y_test_eye = train_test_split(X_eye_balanced, Y_eye_balanced, test_size=0.1)

    y_train = pandas.DataFrame(data=y_train)

    print("eye", X_train_eye.shape, y_train_eye.shape)
    print("pss", X_train.shape, y_train.shape)

    #model.fit([X_train, X_eye_balanced], [y_train, Y_eye_balanced], epochs=1, batch_size=10)

    pipeline.fit([X_train, X_train_eye], [y_train, y_train_eye])

    prediction_tmp = pipeline.predict([X_test, X_test_eye])
...
print("=================")
print("total acuracia %s" % (total_acuracia/folds))
print("total fscore %s" % (total_fscore/folds))
print("=================")
